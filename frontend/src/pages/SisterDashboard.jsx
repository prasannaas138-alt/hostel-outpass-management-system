...
import {
  IconHome, IconClock, IconCheck, IconUsers, IconAlert, IconMenu,
  IconClose, IconHistory, IconUser, IconLogout, IconSearch,
...
export default function SisterDashboard() {
  ...
  const cards = [
    ...
    { label: 'Pending Approvals', value: stats.pendingApprovals, sub: 'Need your attention', icon: IconClock, tone: 'wd-card-icon--amber' },
    { label: 'Approved Today', value: stats.approvedToday, sub: 'Outpasses issued', icon: IconCheck, tone: 'wd-card-icon--green' },
    { label: 'Expired Outpasses', value: stats.expiredOutpasses, sub: 'Not returned yet', icon: IconAlert, tone: 'wd-card-icon--red' },
  ];
  ...
  return (
    <div className="wd-shell ss-shell">
      ...
      <aside className={drawerOpen ? 'wd-sidebar wd-sidebar--open' : 'wd-sidebar'} aria-label="Sister navigation">
        <div className="wd-side-brand"><img src="/st-joseph-logo.png" alt="St. Joseph's University" /><div><strong>St. Joseph's University</strong><span>Hostel Office · H.O.M.S</span></div></div>
        <div className="wd-side-user"><span className="wd-avatar">{initial}</span><div className="wd-user-meta"><strong>{user?.name || 'Sister'}</strong><span>Sister</span></div></div>
        <nav className="wd-nav" aria-label="Sister sections">
          <button type="button" className="wd-nav-item wd-nav-item--active" onClick={() => goTo('sister-dashboard')}><IconHome size={18} />Dashboard</button>
          <button type="button" className="wd-nav-item" onClick={() => goTo('sister-history')}><IconHistory size={18} />Outpass History</button>
          <button type="button" className="wd-nav-item" onClick={() => { setDrawerOpen(false); setProfileOpen(true); }}><IconUser size={18} />Profile</button>
        </nav>
        <button className="wd-side-logout" type="button" onClick={logout}><IconLogout size={18} />Logout</button>
      </aside>

      <div className="wd-body">
        <header className="wd-topbar">
          <div className="wd-topbar-main"><img className="wd-topbar-logo" src="/st-joseph-logo.png" alt="St. Joseph's University" /><div className="wd-topbar-brand"><strong>St. Joseph's University</strong><span>Hostel Office · H.O.M.S</span></div><button className="wd-burger" type="button" aria-label={drawerOpen ? 'Close menu' : 'Open menu'} aria-expanded={drawerOpen} onClick={() => setDrawerOpen((open) => !open)}>{drawerOpen ? <IconClose size={20} /> : <IconMenu size={20} />}</button></div>
          <div className="wd-topbar-user"><span className="wd-avatar">{initial}</span><div className="wd-user-meta"><strong>{user?.name || 'Sister'}</strong><span>Sister</span></div><NotificationBell /></div>
        </header>

        <main className="wd-main" id="sister-dashboard">
          <section className="wd-greet ss-welcome"><div><p className="wd-greet-eyebrow">Sister Dashboard</p><h1>Hello, {firstName}</h1><p>Review HOD-approved Home Outpass requests and track their status.</p></div><span className="wd-date-chip">{today}</span></section>
          <div className="wd-cards">{cards.map(({ label, value, sub, icon: Icon, tone }) => <article className="wd-card" key={label}><span className={`wd-card-icon ${tone}`}><Icon size={20} /></span><p className="wd-card-label">{label}</p><p className="wd-card-value">{value ?? '—'}</p><p className="wd-card-sub">{sub}</p></article>)}</div>
